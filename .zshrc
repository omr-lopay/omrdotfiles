...
_sloth_setup() {
  ...
  LPY_PROMPT=$(_sloth_prompt_style)
  LPY_USE_BAT=$(_sloth_prompt "Use bat as cat replacement (syntax highlighting)" "$LPY_USE_BAT")
  LPY_USE_EZA=$(_sloth_prompt "Use eza as ls replacement (colors, grouping)" "$LPY_USE_EZA")
  LPY_EZA_ICONS=$(_sloth_prompt "Show icons in eza output (needs Nerd Font on your terminal)" "$LPY_EZA_ICONS")
  LPY_SYNTAX_HIGHLIGHTING=$(_sloth_prompt "Enable zsh syntax highlighting" "$LPY_SYNTAX_HIGHLIGHTING")
  LPY_GIT_ALIASES=$(_sloth_prompt "Enable git aliases (gs, ga, gc, gp, ...)" "$LPY_GIT_ALIASES")
  LPY_GIT_FZF=$(_sloth_prompt "Enable git fzf helpers (gcof, grf, glf)" "$LPY_GIT_FZF")
  LPY_CLIPBOARD=$(_sloth_prompt "Enable clipboard integration (xclip)" "$LPY_CLIPBOARD")

  cat > "$LPY_CONF" <<EOF
# Lopay shell preferences — generated by 'sloth setup'
...
_sloth_config() {
  ...
  if [[ -f "$LPY_CONF" ]]; then
    ...
    printf " ${DIM}(defaults — run 'sloth setup' to customise)${RESET}\n\n"
  fi

  printf "  prompt style      \033[1m%s\033[0m\n" "$LPY_PROMPT"
  printf "  bat as cat        %b\n" "$(_sloth_label "$LPY_USE_BAT")"
  printf "  eza as ls         %b\n" "$(_sloth_label "$LPY_USE_EZA")"
  printf "  eza icons         %b\n" "$(_sloth_label "$LPY_EZA_ICONS")"
  printf "  syntax highlight  %b\n" "$(_sloth_label "$LPY_SYNTAX_HIGHLIGHTING")"
  printf "  git aliases       %b\n" "$(_sloth_label "$LPY_GIT_ALIASES")"
  printf "  git fzf helpers   %b\n" "$(_sloth_label "$LPY_GIT_FZF")"
  printf "  clipboard         %b\n" "$(_sloth_label "$LPY_CLIPBOARD")"
  echo
}
...
_sloth_tunnel() {
  ...
  if [[ -z "$port" ]]; then
    ...
    local tunnel_url=""
    [[ -f "$HOME/.tunnel-url" ]] && tunnel_url=$(cat "$HOME/.tunnel-url")

    if [[ -z "$tunnel_url" ]]; then
      printf "\033[31mNo tunnel running.\033[0m\n"
      return 1
    fi

    printf "\n${BOLD}HTTPS Tunnel${RESET}\n\n"
    # Parse Caddyfile for active routes
    printf "  ${DIM}:8080${RESET}    ${tunnel_url}\n"
    grep -oP 'handle_path /:\K[0-9]+' /etc/caddy/Caddyfile 2>/dev/null | while read -r p; do
      printf "  ${DIM}:${p}${RESET}    ${tunnel_url}/:${p}/\n"
    done
    echo
    printf "${DIM}Add a port: sloth tunnel <port>${RESET}\n\n"
    return 0
  fi

  # Validate port
  if ! [[ "$port" =~ ^[0-9]+$ ]]; then
    printf "\033[31mUsage: sloth tunnel <port>\033[0m\n"
    return 1
  fi

  # Check if already in Caddyfile
  if grep -q "handle_path /:${port}/" /etc/caddy/Caddyfile 2>/dev/null; then
    local tunnel_url=""
    [[ -f "$HOME/.tunnel-url" ]] && tunnel_url=$(cat "$HOME/.tunnel-url")
    printf "Port ${port} already exposed: ${tunnel_url}/:${port}/\n"
    return 0
  fi

  # Add new route to Caddyfile (before the default handle block)
  sudo sed -i "/handle {/i\\  handle_path /:${port}/* {\n    reverse_proxy localhost:${port}\n  }" /etc/caddy/Caddyfile
  sudo systemctl reload caddy

  local tunnel_url=""
  [[ -f "$HOME/.tunnel-url" ]] && tunnel_url=$(cat "$HOME/.tunnel-url")
  printf "\033[32m●\033[0m Port ${port} exposed: ${tunnel_url}/:${port}/\n"
}
...
sloth() {
  case "${1:-}" in
    ...
    setup)  _sloth_setup ;;
    config) _sloth_config ;;
    tunnel) _sloth_tunnel "$2" ;;
    reset)
      rm -f "$LPY_CONF"
      printf "\033[1mReset to defaults.\033[0m Restart your shell or run: \033[1msource ~/.zshrc\033[0m\n"
      ;;
    help|*)
      printf "\n\033[1msloth\033[0m — Lopay dev environment\n\n"
      printf "  \033[1msloth init\033[0m            First-time setup wizard\n"
      printf "  \033[1msloth setup\033[0m           Configure shell preferences\n"
      printf "  \033[1msloth config\033[0m          Show current preferences\n"
      printf "  \033[1msloth tunnel\033[0m          Show tunnel URLs\n"
      printf "  \033[1msloth tunnel <port>\033[0m   Expose a port via HTTPS tunnel\n"
      printf "  \033[1msloth reset\033[0m           Reset preferences to defaults\n"
      printf "  \033[1msloth help\033[0m            Show this help\n"
      echo
      ;;
  esac
}
...
case "$LPY_PROMPT" in
  minimal)
    ...
    ;;
  full)
    PROMPT='%F{cyan}%n@%m%f:%F{yellow}%~%f %F{green}${vcs_info_msg_0_}%f%F{dim}$(_sloth_node_version)%f %# '
    ;;
  *)  # standard
